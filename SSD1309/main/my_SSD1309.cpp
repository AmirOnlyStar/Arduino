...
unsigned char SSD1309_buffer[buffer_size];
...
void OLED_print_char(unsigned char x_pos, unsigned char y_pos, unsigned char ch)
{
    ...
    for(s = 0x00; s < 0x06; s++)
    {
      ...
      value  = pgm_read_byte (&font_regular[chr][s]);
      // value = font_regular[chr][s];
      SSD1309_buffer[(page*128)+(x_pos)+i] = value;
      ...
      // SSD1309_buffer[((page * x_max) + x_pos)] = value;
      // SSD1309_buffer[((page * x_max) +x_pos+ i)] = value;
      // i++;
    }
}
...
void Draw_Pixel(unsigned char x_pos, unsigned char y_pos, unsigned char colour)
{
    ...
    value = SSD1309_buffer[((page * x_max) + x_pos)];
